...
class ListingAcquisitionHelper:
    ...
    def acquire_listing_page(
        ...
        decode_html_fn: Any,
        html_parser: Any,
    ) -> Tuple[Optional[str], AcquisitionResult, MonitoringFact, Optional[str]]:
        listing_attempt_id = f"{session_id}_listing"
        listing_url = source.base_url
        now_iso = datetime.now().isoformat()

        try:
            listing_resp = http_getter(listing_url)
            ...
            return None, acq_res, monitoring_fact, None
        ...
        if list_status >= 400:
            ...
            return None, listing_acq_res, monitoring_fact, raw_listing_path
        ...
        monitoring_fact = MonitoringFact(
            ...
            checked_url=list_final_url,
            checked_at=now_iso,
            metadata={"attempt_id": listing_attempt_id, "detail_discovered": bool(detail_url)},
        )
        return detail_url, listing_acq_res, monitoring_fact, raw_listing_path
