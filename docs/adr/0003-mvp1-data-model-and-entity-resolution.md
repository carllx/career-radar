...
1. **确定性候选检索层（Helper Candidate Retrieval）**：
   - 检索层以 **高召回（High Recall）** 为首要目标，仅用于帮助 Agent 缩小合理的历史比对范围；
   - 严禁将职位关键词、标题相似度等作为候选入围的必要条件或硬过滤门槛（Hard Gate），绝不能因关键词不匹配而排除本应由 Agent 审阅的潜在同质候选；
   - MVP-1 阶段优先采用简单可靠的策略（如向 Agent 提供同一用人机构的近期/活跃 Opportunity 宽候选集）；
   - 不锁定固定 Top-N 上限、阈值或复杂算法，Agent 始终拥有自主指令 Helper 扩大历史检索范围的权威；
